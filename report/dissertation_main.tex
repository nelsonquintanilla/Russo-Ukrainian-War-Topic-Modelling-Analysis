...
To train our LDA model we need to provide our model with different parameters, from which, one of the most important ones is the number of topics we want to extract from our dataset. 

Although we can use LDA to successfully learn themes from a collection of documents, LDA analysis requires the number of topics to be predetermined. Furthermore, choosing the number of topics in LDA is typically determined by assessing the perplexity of numerous models built with varying values of number of topics as a parameter \citep{blei_probabilistic_2010}.

We then proceed to describe how we did this and how we tuned the rest of the parameter for our model.
